...
--Rect used: x = 0 and y = 0 then cc.rect(w,h)
function cc.rect(_x,_y,_width,_height)
    if _width == nil and _height == nil then
        return { x = 0, y = 0, width = _x, height = _y }
    end
...
function cc.removeSpriteFrameByImageName(imageName)
    sharedSpriteFrameCache:removeSpriteFrameByName(imageName)
    sharedDirector:getTextureCache():removeTextureForKey(imageName)
end
